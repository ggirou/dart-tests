...
class TodoController {
  ...
  markAllDone() => items.forEach((item) => item.done = true);

  archiveDone() => items.removeWhere((item) => item.done);

  classFor(Item item) => item.done ? 'done' : '';

  remaining() => items.where((item) => item.done).length;
}
